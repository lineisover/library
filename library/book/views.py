...
def book(request, id):
    try:
        ...
        return HttpResponseNotFound(
            "<h2>Книги с id: {0} не существует в библиотеке.</h2>".format(id)
        )
...
def author(request, slug_author):
    try:
        ...
        return HttpResponseNotFound(
            "<h2>Книг автора: {0} не существует в библиотеке.</h2>".format(author)
        )
...
def filter(request, filter):
    ...
    if filter not in FILTER_LIST:
        return HttpResponseNotFound(
            "Фильтрация по {0} невозможна, существуют фильтры: {1}".format(
                filter, *FILTER_LIST
            )
        )
